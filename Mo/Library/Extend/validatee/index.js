...
$validatee.fn.check = function() {
	return this.validate(this.currentnames);
};
/****************************************************
'@DESCRIPTION:	addRule
...
'****************************************************/
$validatee.fn.validate = function(fields) {
	this.exception = "";
	var succeed = true,
		data = this.data,
		name, 
		loop = fields || this.names, len = loop.length;
	for (var i=0;i< len;i++) {
		name = loop[i];
		...
		if (Rules["___exp"] != "" && typeof Rules["___exp"] == "object") {
			if (!Rules["___exp"].test(data[name])) {
				succeed = false;
